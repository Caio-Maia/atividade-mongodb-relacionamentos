...
db.resenha.insertOne({
  ...
  curtidas: 42,
  comentarios: [
    { usuario_id: "usuario_bob", texto: "Concordo plenamente!", data: new Date("2024-04-11") }
  ]
});
...
printjson(db.livro.findOne({ _id: "livro_ocortico" }));

print('\n"Ver todas as resenhas" = 2a query paginada na colecao resenha:');
printjson(db.resenha.find({ livro_id: "livro_ocortico" }).sort({ data: -1 }).limit(5).toArray());
...
const atualizado = db.livro.findOne({ _id: "livro_ocortico" }, { soma_notas: 1, total_resenhas: 1 });
db.livro.updateOne(
  { _id: "livro_ocortico" },
...
// Caso normal: usuario com poucos seguidores — lista embutida
...
// Simulamos 500 seguidores; os 100 mais recentes ficam embutidos
const seguidoresVirais = [];
for (let i = 1; i <= 500; i++) seguidoresVirais.push("usuario_" + i);

db.usuario.insertOne({
  _id: "usuario_clarice",
  nome: "Clarice Lispector Fan Page",
  // Guardamos os 100 MAIS RECENTES embutidos (hot path)
  seguidores: seguidoresVirais.slice(-100),   // ultimos 100: usuario_401..usuario_500
...
const viral = db.usuario.findOne({ _id: "usuario_clarice" }, { nome: 1, seguidores_count: 1, has_extras: 1, _id: 0 });
printjson(viral);
print("  Seguidores na colecao extras: " + db.seguidores_extras.countDocuments({ usuario_id: "usuario_clarice" }));
